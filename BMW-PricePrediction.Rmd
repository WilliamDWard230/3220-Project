...
# filter out all vehicles with engine size of 0
df_filtered <- df_filtered[df_filtered$engineSize != 0, ]
...
train_index = createDataPartition(df_filtered$price, p=0.8, list=FALSE)
df_train <- df_filtered[ train_index, ]
df_test <- df_filtered[ -train_index, ]
```

Train GAM model
```{r}

gam_model <- gam(price ~ model + year + transmission + mileage + fuelType + tax + mpg + engineSize, data=df_train)
...
Train RF
```{r}
rf <- randomForest(price ~ model + year + transmission + mileage + fuelType + tax + mpg + engineSize, data=df_train)

summary(rf)

predictions <- predict(rf, df_test)
summary(predictions)

resis <- abs(predictions - df_test$price)
summary(resis)
...
importance[ordered(importance$Overall), ]

summary(lm_model)

preds <- predict(lm_model, df_test)
targets <- df_test$price

resis <- abs(targets - predictions)
...
x_test <- model.matrix(
  price ~ model + price + year + transmission + mileage + fuelType + tax + mpg + engineSize,
  data = df_test
)


# https://www.geeksforgeeks.org/r-language/xgboost-in-r-programming/
Dtrain = xgb.DMatrix(
        data = x_train,
...
importance <- xgb.importance(
  feature_name = colnames(x_train),
  model = xgb_model
)

xgb.plot.importance(importance, col="steelblue", main="Feature Importance for XGB Model")
...
resis <- predictions - df_test$price
summary(resis)

```

Predict Franklins car with RF
```{r}
# Franklin's Car eval
predictions <- predict(rf, data.frame(
  model=c("2 Series"),
  year=c(2015),
  transmission=c("Semi-Automatic"),
  mileage=c(71000),
  fuelType=c("Petrol"),
  tax=c(0),
  mpg=c(18),
